...
What would you like help with today?`,
    ...
    useEffect(() => {
        if (currentMessage.length > 2) {
            ...
            const suggestions = TAX_SUGGESTION_KEYWORDS.filter(
                (keyword) => keyword.toLowerCase().includes(lastWord) && keyword.toLowerCase() !== lastWord
            ).slice(0, 5)
        ...
        async (files: FileList) => {
            Array.from(files).forEach(async (file) => {
                ...
                setTimeout(async () => {
                    ...
                    setMessages((prev) =>
                        prev.map((msg) =>
                            msg.id === attachmentMessage.id ? { ...msg, attachments: [processedAttachment] } : msg
                        )
                    )
...
Would you like me to help you understand how to use this information on your tax return?`,
    ...
    const renderMessage = (message: ChatMessage, index: number) => {
        ...
        return (
            <div key={message.id} className={`flex ${isUser ? 'justify-end' : 'justify-start'} mb-4 group`}>
                <div
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center space-x-2">
                                <span className="text-xs font-medium">{isUser ? 'You' : 'Tax Assistant'}</span>
                            ...
                            <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                >
                                    <Bookmark
                                        className={`w-3 h-3 ${message.metadata?.isBookmarked ? 'fill-current' : ''}`}
                                    />
                                </Button>
                                            ...
                                            {attachment.ocrData && (
                                                <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded text-xs">
                                                    <div className="flex items-center space-x-1 text-green-700 mb-1">
                                                        ...
                                                        <Badge variant="secondary" className="text-xs">
                                                            {Math.round(attachment.ocrData.confidence * 100)}%
                                                            confidence
                                                        </Badge>
                                                    </div>
    ...
    return (
        <div
            className={`flex flex-col h-full bg-background border rounded-lg ${isExpanded ? 'fixed inset-4 z-50' : ''}`}
        >
            {/* Header */}
            <div className="flex items-center justify-between p-4 border-b">
                ...
                <div className="flex items-center space-x-2">
                    ...
                    <Button size="sm" variant="outline" onClick={() => setPiiDetectionEnabled(!piiDetectionEnabled)}>
                        {piiDetectionEnabled ? (
            ...
            >
                <div
                    className={`${dragActive ? 'border-2 border-dashed border-primary bg-primary/5 rounded-lg p-8' : ''}`}
                >
                    {dragActive && (
            ...
            <div className="p-4 border-t">
                <div className="flex items-end space-x-2">
                    ...
                    <Button onClick={sendMessage} disabled={!currentMessage.trim() || assistantTyping} className="h-10">
